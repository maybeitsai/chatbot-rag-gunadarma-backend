...
class DataProcessor:
        ...
        try:
            ...
            if not data_file:
                self.tracker.log_step(
                    SetupStep.DATA_PROCESS, False, "No data file found"
                )
                return False
            ...
            if not documents:
                self.tracker.log_step(
                    SetupStep.DATA_PROCESS, False, "No valid documents"
                )
                return False
    ...
    def _convert_to_documents(self, cleaned_data: List[Dict[str, Any]]) -> List:
        ...
        documents = []
        self.console.print(
            f"📄 Converting {len(cleaned_data)} items to Document objects..."
        )
        ...
        self.tracker.log_step(
            SetupStep.DATA_PROCESS, True, f"Converted {len(documents)} documents"
        )
        return documents
    ...
    def _setup_vector_store(self, documents: List) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.console.print(
                    f"⚠️ Index creation failed (non-critical): {index_error}",
                    style="yellow",
                )
